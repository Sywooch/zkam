...
class DefaultController extends Controller
{
    ...
    public function actionIndex($city='',$user='',$cat1='')
    {
        if (!empty($city)) $from_city = GeobaseCity::findOne(['url'=>$city]); else $from_city='';
        if (!empty($cat1)) $cat = ModArendaTree::findOne(['url'=>$cat1]); else $cat='';
        $model = new Ads();
        $data = $model->Adss($from_city,$user,$cat);
    ...
    {
        $id = substr($id,strrpos($id,'-')+1,strlen($id)-strrpos($id,'-'));
        if (!is_numeric($id)) exit('страница не найдена!');
        ...
        $result = Yii::$app->db
            ->createCommand($query)
            ->queryOne();
        if (!$result) (exit('страница не найдена!'));
